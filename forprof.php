...
<body>
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        include "cob.php"; // Conexión a la base de datos

        // Obtener los datos del formulario
        $nombre_usuario = $conn->real_escape_string($_POST['nombre_usuario']);
        $contraseña = $conn->real_escape_string($_POST['contraseña']);

        // Consulta para verificar el nombre de usuario, la contraseña y si el usuario está activo
        $sql = "SELECT * FROM profesor WHERE nombre_usuario = '$nombre_usuario' AND contraseña = '$contraseña' AND activo = 1";
        $result = $conn->query($sql);

        if ($result->num_rows > 0) {
            // Las credenciales son correctas y el usuario está activo, redirigir a profesores.php
            header("Location: profesores.php");
            exit(); // Asegúrate de usar exit después de header para detener la ejecución del script
        } else {
            // Las credenciales son incorrectas o el usuario está inhabilitado
